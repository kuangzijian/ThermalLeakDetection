...
class STAE(nn.Module):
    ...
    def forward(self, inp, hidden=None):
        ...
        # print(x.shape)
        x = x.reshape(batch_size,seq_len,32,23,23)
        
        if hidden==None:

            h_t1,c_t1 = self.init_hidden_states(batch_size,16,23,23)
            h_t2,c_t2 = self.init_hidden_states(batch_size,16,23,23)
            h_t3,c_t3 = self.init_hidden_states(batch_size,32,23,23)
        ...
        x = torch.reshape(x,[batch_size*seq_len,32,23,23])
